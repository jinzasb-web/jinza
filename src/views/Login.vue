...
<style scoped>
.login-page { position:relative; min-height:100vh; display:flex; align-items:center; justify-content:center; padding:24px; overflow:hidden; }
.login-bg { position:fixed; inset:0; z-index:0; overflow:hidden; }
.login-bg::before {
  content: '';
  position: absolute; inset: 0;
  background: linear-gradient(135deg, #0d2f55 0%, #0a2540 40%, #051525 100%);
}
.login-bg::after {
  content: '';
  position: absolute; inset: 0;
  background:
    radial-gradient(ellipse at 20% 50%, rgba(141,175,80,0.08) 0%, transparent 50%),
    radial-gradient(ellipse at 80% 20%, rgba(238,128,37,0.06) 0%, transparent 50%);
}
.bg-shapes { position:absolute; inset:0; }
.shape { position:absolute; border-radius:50%; }
.shape-1 {
  width: 500px; height: 500px; top: -100px; right: -80px;
  background: radial-gradient(circle, rgba(141,175,80,0.12) 0%, transparent 70%);
  animation: float1 12s ease-in-out infinite;
}
.shape-2 {
  width: 400px; height: 400px; bottom: -100px; left: -60px;
  background: radial-gradient(circle, rgba(238,128,37,0.10) 0%, transparent 70%);
  animation: float2 16s ease-in-out infinite;
}
.shape-3 {
  width: 200px; height: 200px; top: 50%; left: 40%;
  background: radial-gradient(circle, rgba(141,175,80,0.06) 0%, transparent 70%);
  animation: float1 10s ease-in-out infinite reverse;
}
@keyframes float1 { 0%,100%{transform:translate(0,0) scale(1)} 50%{transform:translate(-30px,25px) scale(1.05)} }
@keyframes float2 { 0%,100%{transform:translate(0,0) scale(1)} 50%{transform:translate(25px,-30px) scale(1.08)} }
.login-container { position:relative; z-index:1; width:100%; max-width:380px; animation:cardIn .7s cubic-bezier(.22,.98,.34,1.02) both; }
...
.login-form :deep(.el-input__wrapper) { background:rgba(255,255,255,.08)!important; border-radius:12px; box-shadow:0 0 0 1px rgba(255,255,255,.12) inset; transition:all .25s; }
.login-form :deep(.el-input__wrapper:hover) { box-shadow:0 0 0 1px #8daf50 inset; }
.login-form :deep(.is-focus.el-input__wrapper) { box-shadow:0 0 0 2px #ee8025 inset!important; background:rgba(255,255,255,.12)!important; }
.login-form :deep(.el-input__inner) { color:#fff; font-weight:500; }
.login-form :deep(.el-input__inner::placeholder) { color:rgba(255,255,255,.4); }
.login-form :deep(.el-input__prefix) { color:rgba(255,255,255,.4); }
.login-button {
  ...
  box-shadow: 0 6px 20px -4px rgba(238,128,37,0.4);
  transition: transform .15s, box-shadow .2s;
}
.login-button:hover {
  transform:translateY(-1px);
  box-shadow: 0 8px 28px -6px rgba(238,128,37,0.6);
}
.login-button:active { transform:translateY(0); }
.footer-text { text-align:center; margin-top:24px; padding-top:20px; border-top:1px solid rgba(255,255,255,.06); font-size:11px; color:rgba(255,255,255,.25); letter-spacing:1px; }
</style>
